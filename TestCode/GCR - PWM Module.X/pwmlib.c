...
void BeginPWM()
{
    ...
    //To configure CCP1 for PWM mode, bits 2 and 3 of CCP1CON
    //  must be set;  leave 2 LSBs of duty cycle clear for now
    CCP1CON = 0b00001100;  //sets CCP1 as PWM mode

    //Set the PWM duty cycle by loading CCPR1L register and
    ...
    while(1)  //waits until TMR2IF = 1
    {
        ...
        {
            TMR2IF = 0;             //clear flag
            TRISCbits.TRISC5 = 0;   //set PWM output to start
            break;                  //exit loop
        }
    }
}
